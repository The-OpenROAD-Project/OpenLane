...
First build the PDKs and place them into the docker_build folder:

```bash
cd ..
make PDK_ROOT=`pwd`/docker_build/tar/pdks pdk
cd docker_build/
```

Then builds each tool in a separate docker image (i.e. one docker image per tool) using the make target below. Then we export the binaries generated by each tool's image to tarballs in `tar` directory.
