...
echo $PDK_BASE
echo $RUN_ROOT
docker run -it -v $RUN_ROOT:/openLANE_flow -v $PDK_BASE:/pdks -e PDK_ROOT=/pdks -u $(id -u $USER):$(id -g $USER) openlane:rc3  bash -c "python3 run_designs.py -d $(cat $RUN_ROOT/travisCI/$test_set) -t TEST_$test_set -dl -dt -th 20 -b regression_results/benchmark_results/SW_HD.csv -p 30"
