...
@click.command("replace_pins", help="""
...
def replace_pins(output, input_lef, first_def, reference_def):
    ...
    for first_bterm in first_bterms:
        ...
        if (first_bterm.getSigType() == "POWER") or (
            ...
            print("Bterm", first_name, "is declared as", first_bterm.getSigType(), "ignoring it")
            continue
        all_bterm_names.add(first_name)
    
    print("[defutil.py:replace_pins] Found", len(all_bterm_names), "block terminals in first def")
    ...
    for ref_bterm in ref_bterms:
        ...
        for ref_pin in ref_pins:
            ...
            for box in boxes:
                ...
                ref_bterm_locations[ref_name].append((layer, box.xMin(), box.yMin(), box.xMax(), box.yMax(),))
    ...
    for output_bterm in output_bterms:
        ...
        if name in ref_bterm_locations and name in all_bterm_names:
            ...
            for ref_bterm_location_tuple in ref_bterm_locations[name]:
                ...
                odb.dbBox.create(output_new_bpin, layer,
                    ref_bterm_location_tuple[1], ref_bterm_location_tuple[2], ref_bterm_location_tuple[3], ref_bterm_location_tuple[4])
                output_new_bpin.setPlacementStatus("PLACED")
        else:
            print("[defutil.py:replace_pins] Not found", name, "in reference def, but found in output def. Leaving as-is")
