...
}
 {
    print \$0;
        ...
        if { $::env(STD_CELL_LIBRARY_OPT) != $::env(STD_CELL_LIBRARY)} {
            ...
            # Create power file
            try_catch awk $cvc_power_awk $::env(CURRENT_NETLIST) > $::env(erc_results).power
            # Create cdl file by combining cdl library with lef spice
	        try_catch awk $cvc_cdl_awk $lib_cdl $::env(magic_results).lef.spice \
                > $::env(erc_results).cdl
            try_catch cvc $::env(SCRIPTS_DIR)/cvc/$::env(PDK)/cvcrc.$::env(PDK) \
                |& tee $::env(TERMINAL_OUTPUT) [index_file $::env(erc_logs)_screen.log]
            TIMER::timer_stop
