...
proc global_placement_or {args} {
    ...
    TIMER::timer_start
    set ::env(SAVE_DEF) [index_file $::env(placement_tmpfiles)_global.def]
    ...
    set report_tag_saver $::env(placement_reports)_global
    set ::env(placement_reports)_global [index_file $::env(gplace_reports 0]
    try_catch $::env(OPENROAD_BIN) -exit $::env(SCRIPTS_DIR)/openroad/replace.tcl |& tee $::env(TERMINAL_OUTPUT) [index_file $::env(placement_logs)_global.log 0]
    set ::env(placement_reports)_global $report_tag_saver
...
proc random_global_placement {args} {
    ...
    try_catch $::env(OPENROAD_BIN) -python $::env(SCRIPTS_DIR)/random_place.py --lef $::env(MERGED_LEF_UNPADDED) \
        ...
        |& tee $::env(TERMINAL_OUTPUT) [index_file $::env(placement_logs)_global.log 0]
...
proc detailed_placement_or {args} {
    ...
    TIMER::timer_start
    set ::env(SAVE_DEF) $::env(routing_results)_detailed.def

    try_catch $::env(OPENROAD_BIN) -exit $::env(SCRIPTS_DIR)/openroad/opendp.tcl |& tee $::env(TERMINAL_OUTPUT) [index_file $::env(routing_logs)_detailed.log]
    set_def $::env(SAVE_DEF)

    if {[catch {exec grep -q -i "fail" [index_file $::env(routing_logs)_detailed.log 0]}] == 0}  {
	puts_info "Error in detailed placement"
	puts_info "Retrying detailed placement"
	set ::env(SAVE_DEF) $::env(routing_results)_detailed.1.def

	try_catch $::env(OPENROAD_BIN) -exit $::env(SCRIPTS_DIR)/openroad/opendp.tcl |& tee $::env(TERMINAL_OUTPUT) [index_file $::env(routing_logs)_detailed.log]
    }

    if {[catch {exec grep -q -i "fail" [index_file $::env(routing_logs)_detailed.log 0]}] == 0}  {
	puts "Error: Check [index_file $::env(routing_logs)_detailed.log 0]"
	puts stderr "\[ERROR\]: Check [index_file $::env(routing_logs)_detailed.log 0]"
	exit 1
    }
...
proc run_resizer_timing {args} {
    if { $::env(PL_RESIZER_TIMING_OPTIMIZATIONS) == 1} {
        ...
        TIMER::timer_start
        set ::env(SAVE_DEF) [index_file $::env(placement_tmpfiles)_rsz_timing.def 0]
        set ::env(SAVE_SDC) [index_file $::env(placement_tmpfiles)_rsz_timing.sdc 0]
        try_catch $::env(OPENROAD_BIN) -exit $::env(SCRIPTS_DIR)/openroad/resizer_timing.tcl |& tee $::env(TERMINAL_OUTPUT) [index_file $::env(placement_logs)_rsz_timing_optimization.log 0]
        set_def $::env(SAVE_DEF)
        ...
        write_verilog $::env(placement_results)_rsz_optimized.v
        set_netlist $::env(placement_results)_rsz_optimized.v
...
proc run_resizer_design {args} {
    if { $::env(PL_RESIZER_DESIGN_OPTIMIZATIONS) == 1} {
        ...
        TIMER::timer_start
        set ::env(SAVE_DEF) [index_file $::env(placement_tmpfiles)_rsz.def 0]
        set ::env(SAVE_SDC) [index_file $::env(placement_tmpfiles)_rsz.sdc 0]
        try_catch $::env(OPENROAD_BIN) -exit $::env(SCRIPTS_DIR)/openroad/resizer.tcl |& tee $::env(TERMINAL_OUTPUT) [index_file $::env(placement_logs)_rsz_design_optimization.log 0]
        set_def $::env(SAVE_DEF)
        set ::env(CURRENT_SDC) $::env(SAVE_SDC)
