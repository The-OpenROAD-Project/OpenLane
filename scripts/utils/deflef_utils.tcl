...
proc add_lefs {args} {
  ...
  if { $::env(WIDEN_SITE) == 1 && $::env(WIDEN_SITE_IS_FACTOR) == 1 } {
    ...
    try_catch $::env(SCRIPTS_DIR)/merge_lef.py -i $::env(MERGED_LEF) {*}$arg_values(-src) -o $::env(MERGED_LEF).new
    try_catch $::env(SCRIPTS_DIR)/merge_lef.py -i $::env(MERGED_LEF_UNPADDED) {*}$arg_values(-src) -o $::env(MERGED_LEF_UNPADDED).new
    ...
    # The original lef
    try_catch $::env(SCRIPTS_DIR)/merge_lef.py -i $::env(MERGED_LEF_ORIGINAL) {*}$arg_values(-src) -o $::env(MERGED_LEF_ORIGINAL).new
    try_catch $::env(SCRIPTS_DIR)/merge_lef.py -i $::env(MERGED_LEF_UNPADDED_ORIGINAL) {*}$arg_values(-src) -o $::env(MERGED_LEF_UNPADDED_ORIGINAL).new
    ...
    # The modified lef
    try_catch $::env(SCRIPTS_DIR)/merge_lef.py -i $::env(MERGED_LEF_WIDENED) {*}$arg_values(-src) -o $::env(MERGED_LEF_WIDENED).new
    try_catch $::env(SCRIPTS_DIR)/merge_lef.py -i $::env(MERGED_LEF_UNPADDED_WIDENED) {*}$arg_values(-src) -o $::env(MERGED_LEF_UNPADDED_WIDENED).new
