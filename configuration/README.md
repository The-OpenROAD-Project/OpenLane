...
| `SYNTH_ADDER_TYPE` | Adder type to which the $add and $sub operators are mapped to.<br> Possible values are `YOSYS|FA|RCA`; where YOSYS refers to using Yosys's internal definition, FA refers to full-adder structure, and RCA refers to ripple carry adder structure. <br> (Default: `YOSYS`)|
