...
|Variable|Description|
|-|-|
| `VERILOG_FILES_BLACKBOX` | Black-boxed, Verilog files where the implementation is ignored. Useful for pre-hardened macros you incorporate into your design, used during synthesis and sta. `/// sta-blackbox` can be added to a file in order skip that file while doing sta. This will blackbox all the modules defined inside that file. It is recommended to provide a gatelevel netlist whenever possible to do full sta. |
...
|Variable|Description|
|-|-|
| `BOTTOM_MARGIN_MULTIPLIER`     | The core margin, in multiples of site heights, from the bottom boundary. If `FP_SIZING` is absolute and `CORE_AREA` is set, this variable has no effect. <br> (Default: `4`) |
...
| `FP_IO_HORIZONTAL_EXTENSION`  |  Extends the horizontal io pins outside of the die by the specified units<br> (Default: `0` Disabled) |
| `FP_IO_HORIZONTAL_LENGTH`  | The length of the horizontal IOs in microns. <br> (Default: `4`) |
| `FP_IO_HORIZONTAL_THICKNESS_MULTIPLIER`  | A multiplier for horizontal pin thickness. Base thickness is the pins layer minwidth <br> (Default: `2`) |
...
| `FP_IO_UNMATCHED_ERROR`  | Exit on unmatched pins in a provided `FP_PIN_ORDER_CONFIG` file. 0=Disable 1=Enable. <br> (Default: `1` Enabled) |
| `FP_IO_VERTICAL_EXTENSION`  |  Extends the vertical io pins outside of the die by the specified units<br> (Default: `0` Disabled) |
| `FP_IO_VERTICAL_LENGTH`  | The length of the vertical IOs in microns. <br> (Default: `4`) |
| `FP_IO_VERTICAL_THICKNESS_MULTIPLIER`  | A multiplier for vertical pin thickness. Base thickness is the pins layer minwidth <br> (Default: `2`) |
| `FP_PADFRAME_CONFIG`  | A configuration file passed to padringer, a padframe generator. <br> (Default: NONE) |
...
| `FP_PDN_CONFIG` | Points to a pdn configuration file that describes how to construct the pdn in detail.  <br> (Default: `scripts/openroad/common/pdn_config.tcl`) |
| `FP_PDN_ENABLE_GLOBAL_CONNECTIONS`  | Enables power connection to std cells. It is rare that this variable need to be disabled <br> (Default: `1`) |
...
| `FP_TAPCELL_DISTANCE`  | The horizontal distance between two tapcell columns <br> (Default: `14`) |
...
| `LEFT_MARGIN_MULTIPLIER`       | The core margin, in multiples of site widths, from the left boundary. If `FP_SIZING` is absolute and `CORE_AREA` is set, this variable has no effect.  <br> (Default: `12`) |
| `RIGHT_MARGIN_MULTIPLIER`      | The core margin, in multiples of site widths, from the right boundary. If `FP_SIZING` is absolute and `CORE_AREA` is set, this variable has no effect.   <br> (Default: `12`) |
...
| `TOP_MARGIN_MULTIPLIER`        | The core margin, in multiples of site heights, from the top boundary. If `FP_SIZING` is absolute and `CORE_AREA` is set, this variable has no effect. <br> (Default: `4`) |
...
These variables worked initially, but they were too sky130 specific and will be removed. Currently, if you define them in your design, they'll be used, but it's recommended to update your configuration to use `FP_IO_HORIZONTAL_LAYER` and `FP_IO_VERTICAL_LAYER`, which are defined in the PDK.
...
| `MACRO_PLACEMENT_CONFIG` | Specifies the path a file specifying how openlane should place certain macros |
