...
.. warning::
    If ``DIE_AREA`` is set to big value then you are going to have routing/placement/timing issues.
...
In this section, we will use manual macro placement to place the macros.
...
.. warning::
    ...
    OpenRAM has special set of DRC rules. The current DRC deck is missing these rules, causing false issues.
    Do not use this configurations 
...
However, you will still get DRCs. This DRCs are not critical.
