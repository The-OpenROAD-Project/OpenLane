...
def verify_versions(no_tools: bool = False, report_file=sys.stderr):
    ...
    try:
        ...
        raise Exception("Flow script tool manifest not found. This is a fatal error.") # Is this even possible?
    ...
    if installed:
        ...
        try:
            container_manifest_path = join(openlane_dir, "dependencies", "tool_metadata.yml")
    ...
    for tool in unmatched_tools_flow:
        ...
        if installed and tool_object.get("in_install") is not None and not tool_object["in_install"]:
            continue
        print(f"Tool {tool} is required by the flow scripts being used, but appears to not be installed in the environment.", file=report_file)
        mismatches = True

    unmatched_tools_container = tool_set_container - tool_set_flow
    for tool in unmatched_tools_container:
        print(f"Tool {tool} is installed in the environment, but has no corresponding entry in the flow scripts.", file=report_file)
        mismatches = True
    ...
    for tool in environment_manifest:
        ...
        if container_commit != flow_script_commit:
            print(f"The version of {tool['name']} installed in the environment does not match the one required by the OpenLane flow scripts (installed: {container_commit}, expected: {flow_script_commit})", file=report_file)
            mismatches = True
