    ...
    def get() -> Optional['ContainerInfo']:
        try:
            ...
            if info.get("host") is not None:
                ...
                # Get Version
                try:
                    version_output = subprocess.check_output(["docker", "--version"]).decode("utf8").strip()
...
class OSInfo(StringRepresentable):
    ...
    def __init__(self):
        ...
        self.kernel_version = platform.release() # Unintuitively enough, it's the kernel's release
        self.distro = None
