...
parser.add_argument('--no-tarball', action="store_true", help="Do not create a tarball.")
args = parser.parse_args()

script_path = abspath(args.or_script)
run_path = abspath(args.run_path)
pdk_root = abspath(args.pdk_root)
tarball = not args.no_tarball
...
# Phase 5: Create Run File
run_cmd = join(destination_folder, "run")
...
if tarball:
    last_output = destination_folder + ".tar.gz"
    os.system(f"tar -cv {destination_folder} | xz | gzip > {last_output}")
